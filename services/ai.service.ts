import { AIProjectGeneratorRequest, AIGeneratedProject, AIGeneratedMilestone, AIGeneratedTask, TaskPriority } from "../types";
...
IMPORTANT: Return ONLY valid JSON, no additional text or markdown formatting.`;
   ...
   */
  private static validateAndMapResponse(response: any, request: AIProjectGeneratorRequest): AIGeneratedProject {
    ...
    const mappedMilestones: AIGeneratedMilestone[] = response.milestones.map((m: any, index: number) => ({
    ...
    const mappedTasks: AIGeneratedTask[] = response.tasks.map((t: any) => ({
      ...
      milestoneIndex: typeof t.milestoneIndex === "number" && t.milestoneIndex < mappedMilestones.length ? t.milestoneIndex : undefined,
    }));
   ...
   * Also sanitizes common invalid JSON escape sequences like backslash-backticks and backslash-single quotes.
   */
  private static parseJsonResponse(text: string): any {
    let cleaned = text.trim();
    
    // Sanitize common invalid JSON escape sequences
   ...
   */
  static async generateTaskBreakdown(taskTitle: string, taskDescription: string, projectContext: string): Promise<{ subtasks: { title: string; description: string; estimatedDays: number }[]; acceptanceCriteria: string[] }> {
    if (!GEMINI_API_KEY) {
...
IMPORTANT: Return ONLY valid JSON.`;
   ...
   */
  static async analyzeProjectHealth(projectData: { name: string; description: string; totalTasks: number; completedTasks: number; overdueTasks: number; teamSize: number }): Promise<{
    healthScore: number;
...
Return JSON:
{
  ...
  "delayPrediction": "Predicted to be delayed by 1 week due to...",
  "bottlenecks": ["bottleneck 1", "bottleneck 2"]
}

Health Score: 0-100 (100 is excellent)
Risk Level: LOW, MEDIUM, or HIGH
Delay Prediction: 1-2 sentence string predicting if the project will be delayed.
Bottlenecks: Array of identified issues hindering progress.
...
- The suggestedAssigneeId MUST exactly match the id field of one of the provided Team Members, or be null.`;
    ...
    try {
      ...
      if (data.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        textResponse = data.contents[0].parts[0].text;
      } else if (data.text) {
...
${activities
  .slice(0, 20)
  .map((a) => `- ${a.user?.name || "System"} ${a.action} on ${new Date(a.createdAt).toLocaleDateString()}`)
  .join("\n")}
