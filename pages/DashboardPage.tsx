...
export const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    // React.StrictMode runs effects twice in DEV; prevent duplicate Supabase calls.
  ...
  const fetchProjects = async () => {
    setIsLoading(true);
    try {
      console.time("api.projects.list");
      const data = await api.projects.list();
      console.timeEnd("api.projects.list");

      setProjects(data);
    } catch (err: any) {
      console.error("Fetch error:", err);
      if (err?.name === "AbortError") {
        console.error("Fetch aborted (timeout). Check network, Supabase availability, or browser blockers.");
      }
    } finally {
      setIsLoading(false);
    }
