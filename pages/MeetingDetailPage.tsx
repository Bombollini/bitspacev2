  ...
  return (
    <Layout>
      <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
        <button onClick={() => navigate("/meetings")} className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors group text-sm font-medium">
        ...
        <div className="glass-panel p-8 rounded-2xl relative overflow-hidden border border-neon-purple/20">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6 relative z-10">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neon-purple/10 border border-neon-purple/30 text-neon-purple text-xs font-bold uppercase tracking-widest mb-4">{project.name}</div>
              <h1 className="text-3xl font-display font-bold text-white tracking-tight mb-2 drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">{meeting.title}</h1>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-slate-300 mt-6">
                ...
                {meeting.meetingLink && (
                  <a
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-colors"
                  >
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto">
              <button onClick={handleExportPDF} className="flex items-center justify-center gap-2 px-6 py-3 bg-white/5 text-slate-200 border border-white/10 rounded-xl hover:bg-white/10 transition-all font-medium text-sm">
          ...
          <div className="space-y-6">
            {meeting.meetingSummary ? (
                            ...
                            <div className="flex justify-between items-start gap-4 mb-2">
                              ...
                              <button
                                ...
                                className={`flex-shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                                  isCreated ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-neon-blue/10 text-neon-blue hover:bg-neon-blue/20 border border-neon-blue/30"
                                }`}
                              >
                                {isCreated ? (
                                  <>
                            ...
                            {assignee ? (
                              <div className="flex items-center gap-2">
                                {assignee.avatarUrl ? (
                                  ...
                                  <div className="w-5 h-5 rounded-full bg-neon-purple/20 flex items-center justify-center text-[10px] font-bold text-neon-purple">{assignee.name.charAt(0)}</div>
                                )}
              ...
              <div className="glass-panel p-8 rounded-2xl border border-dashed border-white/20 flex flex-col items-center justify-center text-center h-full min-h-[400px]">
                ...
                <p className="text-slate-400 text-sm max-w-sm">Paste your meeting notes on the left and click "Summarize Meeting" to automatically extract key decisions and generate assignable tasks.</p>
              </div>
            )}
          </div>
        </div>
