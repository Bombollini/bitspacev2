...
export const ProjectDetailPage: React.FC = () => {
  ...
  return (
    <Layout>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4">
          <div>
            <div className="flex items-center gap-4 mb-2">
              ...
              {currentUser?.role === UserRole.OWNER && (
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsEditModalOpen(true)} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors border border-transparent hover:border-white/20" title="Edit Project">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={handleDeleteProject} className="p-2 text-rose-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors border border-transparent hover:border-rose-500/30" title="Delete Project">
                    <Trash2 size={18} />
                  </button>
                </div>
              )}
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400">
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10">
                <LayoutGrid size={14} className="text-neon-blue" /> {project.status}
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10">
                <Users size={14} className="text-neon-purple" /> {members.length} members
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10">
                <ActivityIcon size={14} className="text-neon-pink" /> {completionPercentage}% complete
              </span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 text-white font-bold rounded-xl border border-white/20 hover:border-white/50 shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:shadow-[0_0_20px_rgba(20,184,166,0.3)] transition-all text-sm disabled:opacity-50"
            >
              {isGeneratingPlan ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} className="text-emerald-400" />}
              Auto-Plan (AI)
            </button>
      ...
      <div className="mb-8">
        {activeTab === "tasks" && (
          <div className="flex flex-col lg:flex-row gap-4 overflow-x-auto pb-6 animate-fade-in scrollbar-thin">
            {columns.map((col) => (
              <div key={col.status} className="flex-1 min-w-[200px] lg:min-w-[220px] max-w-xs group">
                <div className="flex items-center justify-between mb-4 px-1">
                  <h3 className="font-bold text-white flex items-center gap-2 font-display tracking-wide text-sm">
                    {col.title}
                    <span className="bg-white/10 text-neon-blue px-2 py-0.5 rounded-full text-xs font-mono border border-neon-blue/20">{tasks.filter((t) => t.status === col.status).length}</span>
                  </h3>
                </div>

                <div onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, col.status)} className="space-y-3 min-h-[200px] h-full rounded-2xl transition-all">
                  {tasks
                    .filter((t) => t.status === col.status)
                    .filter((t) => t.title.toLowerCase().includes(searchQuery.toLowerCase()))
                    .map((task) => (
                      <TaskCard key={task.id} task={task} onSelect={handleTaskClick} onDragStart={handleDragStart} />
                    ))}
                  {tasks.filter((t) => t.status === col.status).length === 0 && (
                    <div className="h-24 border-2 border-dashed border-white/10 rounded-2xl flex items-center justify-center text-slate-500 text-sm font-medium bg-white/5 group-hover:border-white/20 transition-colors">No tasks</div>
                  )}
                </div>
              </div>
            ))}
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between text-xs font-semibold mb-1">
                        ...
                        <span className="text-neon-blue">
                          {stat.completed} / {stat.totalAssigned} tasks
                        </span>
                      </div>
                      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                        <div className="h-full bg-neon-blue rounded-full transition-all duration-500 shadow-[0_0_10px_#00f3ff]" style={{ width: `${stat.totalAssigned > 0 ? (stat.completed / stat.totalAssigned) * 100 : 0}%` }}></div>
                      </div>
                    </div>
                  </div>
                ))}
                {memberStats.length === 0 && <p className="text-sm text-slate-500 text-center py-4">No members yet.</p>}
              </div>
            </div>
          </div>
        </div>
              ...
              <div key={member.id} className="p-6 flex items-center justify-between hover:bg-white/5 transition-colors">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {canManageMembers && member.id !== currentUser?.id && (
                    <button onClick={() => handleRemoveMember(member.id)} className="text-xs font-bold text-rose-500 hover:text-rose-400 hover:underline px-2 py-1 rounded hover:bg-rose-500/10 transition-colors">
                      Remove
                    </button>
                  )}
                </div>
      ...
      {activeTab === "activity" && (
        <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
          ...
          {activities.length === 0 && <p className="text-center text-slate-500 py-10">No recent activity.</p>}
        </div>
      )}
      ...
      {/* Edit Project Modal */}
      {project && <NewProjectModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSubmit={handleUpdateProject} initialData={{ name: project.name, description: project.description }} mode="edit" />}
