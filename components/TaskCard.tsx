...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onSelect, onDragStart }) => {
  return (
    ...
    >
      ...
      <h4 className="text-sm font-semibold text-white mb-3 line-clamp-2 group-hover:text-neon-purple transition-colors">{task.title}</h4>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
        ...
        {task.assignee && <img src={task.assignee.avatarUrl} alt={task.assignee.name} title={task.assignee.name} className="w-7 h-7 rounded-full border border-white/10 group-hover:border-neon-purple/50 transition-colors" />}
      </div>
    </div>
  );
};
