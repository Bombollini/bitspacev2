...
import { LayoutDashboard, FolderKanban, User as UserIcon, LogOut, Menu, X, Plus, Bell, Search, Calendar, Shield } from "lucide-react";
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navigation = [
    // For OWNER and regular users, show Dashboard, Projects, Meetings; only ADMIN doesn't show those
    ...
    ...(user && user.role === UserRole.ADMIN ? [{ name: "Admin", href: "/admin", icon: Shield }] : []),
  ...
  return (
    <div className="min-h-screen flex text-slate-200 font-sans">
      {/* Sidebar - Desktop */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 glass-panel border-r-0 border-white/10 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} bg-surface/95 lg:bg-glass`}
      >
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${
                    isActive ? "bg-neon-blue/10 text-neon-blue border border-neon-blue/50 shadow-[0_0_10px_rgba(0,243,255,0.2)]" : "text-slate-400 hover:bg-white/5 hover:text-white hover:border-white/10 border border-transparent"
                  }`}
                >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 scrollbar-thin">{children}</main>
      </div>
    </div>
  );
};
